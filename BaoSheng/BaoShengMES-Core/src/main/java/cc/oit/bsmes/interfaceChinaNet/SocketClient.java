...
//			socket.setSoTimeout(30000);
	...
	public String readCurrentData(){
		...
		for(Map<String,String> order : orders){
			...
			for(EquipParamAcquisition temp: equipParamAcquisitions){
				...
				bbb.put("bDelete", true);
				bbb.put("fValue", temp.getVvalue());
				aaa.put(temp.getTagname(), bbb);
			}
			orderItem.put(order.get("WORK_ORDER_NO"), aaa);
		}
	...
	public String readOrderData(){
		...
		for(SalesOrder salesOrder: list){
			...
			if(customerOrder == null ||salesOrder.getCustomerCompany() == null){
				continue;
			}
			JSONObject order = new JSONObject();
			order.put("CompanyIssue",salesOrder.getCustomerCompany());
			...
			List<SalesOrderItem> itemList = salesOrderItemService.getBySalesOrderId(salesOrder.getId());
			for(SalesOrderItem salesOrderItem:itemList){
				...
				if(customerOrderItem == null || customerOrderItem.getPlanStartDate() == null ||
						customerOrderItem.getPlanFinishDate() == null ||salesOrderItem.getContractLength() == null
						|| salesOrderItem.getCustProductType() == null || salesOrderItem.getCustProductSpec() == null){
					continue;
				}
				JSONObject orderItem = new JSONObject();
				orderItem.put("TimeStart", DateUtils.convert(customerOrderItem.getPlanStartDate(), DateUtils.DATE_FORMAT));
				orderItem.put("TimeFinish", DateUtils.convert(customerOrderItem.getPlanFinishDate(), DateUtils.DATE_FORMAT));
	...
	public String readHistoryData(Date startDate,Date endDate){
		...
		for(Map<String,String> data:historyData){
			...
			for(int i=0;i<hisWorkOrders.length;i++){
				List<Map<String,String>> equipCodes = equipInfoService.getByWorkOrder(hisWorkOrders[i]);
				for(Map<String,String> equipCode:equipCodes){
					...
					for(String tagName : params.split(",")){
						...
						for(String workTime: equipCode.get("WORK_TIME").split(",")){
							Date start_time =  DateUtils.convert(workTime.split("T")[0]);
							Date end_time =  DateUtils.convert(workTime.split("T")[1]);
							//获取码点集
	...
	private void getParamValues(String tagName, Date startDate, Date endDate,
		...
		if (DateUtils.getSecondsDiff(start_time, startDate) > 0
			...
			equipParamHistoryAcquisition.setEndTime(end_time);
			equipParamHistoryAcquisition.setWwcyclecount(DateUtils
					.getSecondsDiff(startDate, start_time).intValue());
			List<EquipParamHistoryAcquisition> list = equipParamHistoryAcquisitionDAO
					.findParamHistory(equipParamHistoryAcquisition);
			for (EquipParamHistoryAcquisition hisData : list) {
				JSONObject paramValue = new JSONObject();
...
//			}
			...
			if (type.equals("current")) {
				...
				while (DateUtils.getMinuteDiff(startDate, endDate) > 60) {
			...
			while (true) {
				try {
					...
					if(type.equals("history")){
						message = this.readHistoryData(times.get(messageCount).get("startDate"), times.get(messageCount).get("endDate"));
					}
					//组装发送数据包
					...
					while (count == 0) {
						count = in.available();
						if(DateUtils.getSecondsDiff(date,new Date())>30){
							break;
						}
					}
					if(count ==0){
...
//					System.arraycopy(receive1, 22, errorMsg, 0, jsLength);
					...
					JSONObject result = (JSONObject) JSONObject.parse(new String(msg, "utf-8"));
					if(!result.getString("idError").equals("0")){
						log.info(DateUtils.convert(new Date(), DateUtils.DATE_TIME_FORMAT)+":"+"返回码不为0!!");
						break;
					}
					++messageCount;
					if(messageCount >= times.size()){
						//更新历史数据发送时间
						if(type.equals("history")){
							dataDic.setMarks(DateUtils.convert(new Date(), DateUtils.DATE_TIME_FORMAT));
